...
internal object TestUtils {
    ...
    fun withOrientation(@LinearLayoutCompat.OrientationMode orientation: Int): Matcher<View> {
        return object : TypeSafeMatcher<View>() {
            override fun describeTo(description: Description) {
                val orientationString = when (orientation) {
                    LinearLayoutCompat.VERTICAL -> "vertical"
                    LinearLayoutCompat.HORIZONTAL -> "horizontal"
                    else -> "unknown"
                }
                description.appendText("with $orientationString orientation")
            }

            override fun matchesSafely(view: View): Boolean {
                if (view !is LinearLayout) return false
